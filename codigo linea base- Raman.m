...
if ~exist('ZnO.txt', 'file') % Comprueba si el archivo existe en la carpeta
    error('El archivo ZnO.txt no se encuentra en la carpeta especificada.');
    % Muestra error si no existe
end
%==============================================================================
% Cargar datos del espectro Raman
data = load('ZnO.txt'); % Carga los datos del archivo como matriz numérica
...
%-------------------------------------------------------------------------------
baselinea = polyval(p_scaled, x_scaled);                
...
%-=======================================================================
figure;
plot(raman_shift, intensity, 'k', 'DisplayName','ZnO.txt'); hold on; 
...
plot(raman_shift, corrected_intensity, 'r', 'LineWidth', 1.5, 'DisplayName','Corregido (Linea Base)');
% Grafica el espectro corregido en rojo
%---------------------------------------------------------------------------------
%--------------------------------------------------------------------------------
xlabel('Desplazamiento Raman (cm^{-1})');  % Etiqueta eje X
ylabel('Intensidad (u.a.)');               % Etiqueta eje Y
%title('Corrección de Línea Base - espectro Raman ZnO'); % Título de la gráfica
